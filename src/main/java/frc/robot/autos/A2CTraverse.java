...
public class A2CTraverse extends SequentialCommandGroup {
  
  public A2CTraverse(Swerve drive, ArmSubsystem s_ArmSubsystem, IntakeSubsystem s_IntakeSubsystem) {
    ...
    addCommands(
      ...
      new PositionArm(s_ArmSubsystem, List.of(highSequence1, highSequence, high)).withTimeout(4), //6 timeout

      new PositionArm(s_ArmSubsystem, ArmPosition.HighPlace).withTimeout(1),
      new IntakeAuto(s_IntakeSubsystem, Constants.AutoConstants.intakeOutAutoConstant).withTimeout(.5),//place cone on top bar
      new IntakeAuto(s_IntakeSubsystem, 0).withTimeout(0.000001),
      new TraverseChargeStation(drive, true, true, false, false, false, 0).alongWith(new PositionArm(s_ArmSubsystem, List.of(midSequence, travelSequence, travel))).withTimeout(5.5),
      new AutoBalanceSetup(drive, true, true).withTimeout(4),
